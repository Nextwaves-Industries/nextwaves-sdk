...
def setup_logging(level: int = DEFAULT_LOG_LEVEL, logger_name: str = "nextwaves_rfid") -> logging.Logger:
    """
...
class UARTConnection:
    ...
    def __init__(self, port: str, baudrate: int = 115200, timeout: float = 0.5, logger: Optional[logging.Logger] = None):
        """
...
class NRNReader:
    ...
    def __init__(self, port: str, baudrate: int, timeout: Optional[float] = None, logger: Optional[logging.Logger] = None):
        """
    ...
    def extract_valid_frames(self, data: bytes) -> list[bytes]:
        ...
        while i < len(data):
            ...
            if crc_calc == crc_recv:
                ...
                self.logger.debug(f" CRC mismatch at index {i}: expected={hex(crc_calc)}, got={hex(crc_recv)}")
    ...
    def Connect_Reader_And_Initialize(self) -> bool:
        try:
            ...
            self.logger.debug(f" Sending STOP command to ensure Idle state...")
            ...
            if not raw:
                self.logger.debug(f" No response received.")
                return False
            ...
            if frame["mid"] in [0x01, MID.STOP_INVENTORY & 0xFF] and frame["data"][0] == 0x00:
                self.logger.info(f" Reader successfully initialized.")
                return True

            self.logger.error(f" Invalid STOP response.")
            return False
        ...
        try:
            ...
            for idx, frame in enumerate(frames):
                ...
                try:
                    ...
                    self.logger.info(f" Protocol List Length: {protocol_list_len}, Data: {protocols}")
                except IndexError:
                    self.logger.info(f" Reader reports no frequencies or protocols.")
    ...
    def Query_Reader_Information(self) -> dict:
        ...
        try:
            ...
            if frame_data['mid'] != 0x00 or frame_data['category'] != 0x01:
                self.logger.error(f" Unexpected MID or Category.")
                return {}
    ...
    def query_reader_power(self) -> dict[int, int]:
        ...
        try:
            self.stop_inventory()
            self.logger.debug(f" Sending Query Reader Power command...")
            ...
            if not raw:
                self.logger.debug(f" No response received from reader.")
                return {}

            frame = self.parse_frame(raw)
            if not frame:
                self.logger.error(f" Failed to parse response frame.")
                return {}
            ...
            if mid == (MID.QUERY_READER_POWER & 0xFF):
                ...
                self.logger.error(f" Unexpected response MID for power query.")
                return {}
    ...
    def configure_reader_power(self, antenna_powers: dict[int, int], persistence: Optional[bool] = None) -> bool:
        ...
        if not antenna_powers:
            self.logger.info(f" No antenna powers provided for configuration.")
            return False
        ...
        for ant_id, power_dbm in antenna_powers.items():
            if not isinstance(ant_id, int) or not isinstance(power_dbm, int):
                self.logger.error(f" Invalid types: antenna ID and power must both be integers. Got ({type(ant_id)}, {type(power_dbm)}).")
                return False
        ...
        for ant_id, power_dbm in antenna_powers.items():
            ...
            if not (0 <= power_dbm <= 33): # Max power is 36dBm [18]
                self.logger.error(f" Invalid power level for antenna {ant_id}: {power_dbm}dBm. Must be between 0 and 36dBm.")
                return False
        ...
        try:
            ...
            if mid == (MID.CONFIGURE_READER_POWER & 0xFF) and len(data) >= 1:
                result_code = data[0]
                if result_code == 0x00:
                    self.logger.info(f" Reader power configured successfully.")
                    return True
                else:
                    ...
                    self.logger.error(f" Failed to configure reader power. Result code: 0x{result_code:02X} ({error_msg})")
                    return False
    ...
    def start_inventory_with_mode(self, antenna_mask, callback=None) -> bool:
        ...
        try:
            ...
            self.logger.info(f" Starting inventory with antenna mask:", antenna_mask)
    ...
    def query_filter_settings(self):
        ...
        try:
            ...
            if len(data) < 3:
                raise ValueError(f" Invalid response data length: {len(data)}")

            repeated_time = int.from_bytes(data[0:2], byteorder='big')
            rssi_threshold = data[2]

            self.logger.info(f" Repeated Tag Filter Time: {repeated_time} ms, RSSI Threshold: {rssi_threshold}")
            return {
                "repeated_time": repeated_time,
                "rssi_threshold": rssi_threshold
            }
        except Exception as e:
            self.logger.error(f" Error querying filter settings: {e}")
            return None
    ...
    def set_filter_settings(self, repeated_time_ms: int = 0, rssi_threshold: int = 0):
        ...
        try:
            ...
            if status == 0x00:
                self.logger.info(f" Filter settings applied successfully: Time={repeated_time_ms} ms, RSSI={rssi_threshold}")
            else:
                error_map = {
    ...
    def stop_inventory(self) -> bool:
        ...
        if hasattr(self, '_inventory_thread') and self._inventory_thread and self._inventory_thread.is_alive():
            ...
            self.logger.info(f" Inventory thread stopped.")
                ...
                for idx, f in enumerate(frames):
                    try:
                        ...
                        if mid == MID.STOP_OPERATION:  # MID=0xFF, response
                            ...
                            if result == 0x00:
                                self.logger.info(f" Reader responded: STOP successful, now IDLE.")
                                return True
                            else:
                                self.logger.error(f" Reader responded: STOP error code {result:#02x}")
                                return False

                        elif mid in NRNReader.all_read_end_mids():
                            reason = data[0] if data else -1
                            if reason == 1:
                                self.logger.info(f" Read end notification: stopped by STOP command.")
                                return True
                            else:
                                self.logger.info(f" Read ended with reason code {reason}, not STOP command.")
        ...
        self.logger.error(f" STOP failed: no valid response or reading end notification after 10 attempts.")
        return False
            ...
            while time.time() < deadline:
                ...
                for f in frames:
                    try:
                        ...
                        self.logger.error(f" Frame parse error: {e}")
                        continue
                    self.logger.debug(f" [WRITE-EPC-TAG] Received frame: MID={resp['mid']:02X}, Data={resp['data'].hex()}")
                    # Success response
                    if resp["mid"] == 0x11:
                        data = resp["data"]
                        result = data[0]
            ...
            while time.time() < deadline:
                ...
                for f in frames:
                    try:
                        ...
                        self.logger.error(f" Frame parse error: {e}")
                        continue
                    self.logger.debug(f" Write-EPC response: MID={resp['mid']:02X}, Data={resp['data'].hex()}")
                    if resp["mid"] == 0x11:
                        result = resp["data"][0]
                        failed_addr = (
    ...
    def write_epc_to_target_auto(
        ...
        """
        self.logger.warning(f" Scanning for tag with EPC '{target_tag_epc}' (up to {scan_timeout}s) …")
        ...
        try:
            ...
            self.logger.debug(f" Writing new EPC '{new_epc_hex}' (full hex: {full_epc_hex}, start_word={start_word}) …")
            ...
            self.logger.info(f"Write EPC result:", result)

            # Optional verification
            if verify and result.get("success"):
                self.logger.info(f" Verifying new EPC …")
                ...
                if verified_event.wait(timeout=1.5):
                    self.logger.info(f" Verification OK — tag now reports new EPC.")
                else:
                    self.logger.info(f"  Write may have succeeded, but tag not seen with new EPC yet.")
                self.stop_inventory()
            elif not result.get("success"):
                self.logger.error(f"  Write failed; no verification performed.")
    ...
    def query_enabled_ant_mask(self) -> int:
        ...
        try:
            ...
            raw = self.uart.receive(64)
            if not raw:
                self.logger.info(f" No response for enabled antenna mask query.")
                return 0

            frames = self.extract_valid_frames(raw)
            if not frames:
                self.logger.debug(f" No valid frames extracted.")
                return 0
            ...
            for f in frames:
                ...
                data = parsed["data"]
                if len(data) < 2:
                    self.logger.error(f" Invalid data length in response.")
                    continue
            ...
            self.logger.debug(f" No MID=0x02 frame found.")
            return 0
        except Exception as e:
    ...
    def select_profile(self, profile_id: int) -> bool:
        ...
        try:
            ...
            raw = self.uart.receive(64)
            if not raw:
                self.logger.info(f" No response from reader.")
                return False
            ...
            if len(data) < 1 or data[0] != profile_id:
                self.logger.error(f" Profile selection failed. Expected ID {profile_id}, got {data[0] if data else 'N/A'}")
                return False
    ...
    def query_rf_band(self):
        ...
        try:
            ...
            raw = self.receive(64)
            parsed = self.parse_frame(raw)
            self.logger.debug(f" Parsed frame: CAT=0x{parsed['category']:02X}, MID=0x{parsed['mid']:02X}, Data={parsed['data'].hex().upper()}")
    ...
    def set_rf_band(self, band_code: int, persist: bool = True):
        ...
        try:
            ...
            self.logger.info(f" Setting RF Band: {RF_BAND_CODES.get(band_code, 'Unknown')} [Persist={'Yes' if persist else 'No'}]")
            self.logger.debug(f" Sending frame: {frame.hex().upper()}")
            self.send(frame)
            ...
            while time.time() - start_time < timeout:
                ...
                if parsed["category"] != CAT_SET_RF_BAND or parsed["mid"] != MID_SET_RF_BAND:
                    self.logger.debug(f" Ignored frame with CAT=0x{parsed['category']:02X}, MID=0x{parsed['mid']:02X} (expecting CAT=0x02, MID=0x03)")
                    continue
                ...
                status = data[0]
                if status == 0x00:
                    self.logger.info(f" RF Band set to {RF_BAND_CODES[band_code]} [Persist={'Yes' if persist else 'No'}]")
                    return True
                else:
                    error_map = {
            ...
            self.logger.warning(f" No valid response for set_rf_band within timeout.")
            return False
    ...
    def _send_beeper_command(self, ring: int, duration: int) -> bool:
        ...
        try:
            ...
            if response["mid"] == (mid & 0xFF):  # Compare only the MID part
                ...
                if result == 0x00:
                    self.logger.info(f" Buzzer control succeeded.")
                    return True
                else:
    ...
    def get_session(self) -> Optional[int]:
        try:
            ...
            raw = self.uart.receive(64)
            if not raw:
                self.logger.info(f" No response for session query.")
                return None
            ...
            data = response["data"]
            if len(data) < 4:
                self.logger.info(f" Response too short for session info.")
                return None
    ...
    def is_idle(self, retry: int = 3, delay: float = 0.3, settle_delay: float = 0.5) -> bool:
        ...
        for attempt in range(retry):
            try:
                ...
                if not raw_response:
                    self.logger.debug(f" Attempt {attempt+1}/{retry}: No response received for Idle check.")
                    time.sleep(delay)
                    continue

                response = self.parse_frame(raw_response)
                if response["mid"] == (MID.STOP_INVENTORY & 0xFF) and response["data"][0] == 0x00:
                    self.logger.info(f" Reader responded with STOP success.")
                    self.logger.warning(f" Waiting {settle_delay}s for hardware to fully settle...")
                    time.sleep(settle_delay)
                    return True
                else:
                    self.logger.info(f" Attempt {attempt+1}/{retry}: Not idle yet. Response: {response}")
                    time.sleep(delay)
        ...
        self.logger.info(f" Reader did not enter Idle state after retries.")
        return False
    ...
    def configure_baseband(self, speed: int, q_value: int, session: int, inventory_flag: int) -> bool:
        ...
        try:
            ...
            self.stop_inventory()
            if not self.is_idle():
                self.logger.info(f" Reader not idle")
                return False
            time.sleep(0.1)
            self.uart.flush_input()
            ...
            frames = self.extract_valid_frames(raw)
            if not frames:
                self.logger.info(f" No valid response")
                return False
            ...
            for f in frames:
                ...
                if resp['mid'] == 0x0B and resp['category'] in (0x01, 0x02):
                    code = resp['data'][0]
                    if code == 0x00:
                        self.logger.info(f" CONFIG_BASEBAND OK")
                        return True
                    else:
                        errors = {
            ...
            self.logger.info(f" No valid CONFIG_BASEBAND reply")
            return False
    ...
    def query_baseband_profile(self) -> dict:
        ...
        try:
            ...
            raw = self.uart.receive(64)
            if not raw:
                self.logger.info(f" No response for baseband profile query.")
                return {}
            ...
            if len(data) < 4:
                self.logger.error(f" Invalid baseband profile length.")
                return {}
...
# === SDK Entry Point ===
def create_reader(port: str, baudrate: int = 115200, timeout: float = 0.5, 
                 log_level: int = logging.INFO) -> NRNReader:
    """
